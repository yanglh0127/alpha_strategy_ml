...
fac_choose = ['50%_eq_1_高频资金流分布_hfmf', 'sharpe_weight_反转因子相关_vp', 'sharpe_weight_1_日间资金流波动_mf',
              ...
              'sharpe_weight_1_日内成交额分布的稳定性_hfvp', 'best1_1_主力单数行为_mf', 'best1_1_日内成交额的自相关_hfvp',
              'sharpe_weight_1_主力流入流出占比_mf', 'sharpe_weight_量价相关性_vp', '50%_eq_1_大单行为_hfmf',
              'all_eq_1_日内不同时段成交量差异_hfvp', 'sharpe_weight_1_反转因子相关_hfvp', 'sharpe_weight_1_日内收益率的分布_hfvp',
              'best1_1_流动性因子改进_hfvp', 'best1_1_高频贝塔_hfvp', 'sharpe_weight_1_高频量价相关性_hfvp',
              'all_eq_1_反转因子改进_hfmf', 'sharpe_weight_情绪因子_vp', 'best1_1_隔夜(或上午)和下午收益率差异_hfvp',
              'best1_1_反转因子改进_mf', 'next_re']
...
plt.show()
coef_param.to_csv(data_pat + '/linear_regress_22/coef_param.csv',encoding='gbk')
r2_param.to_csv(data_pat + '/linear_regress_22/r2_param.csv',encoding='gbk')
...
fac['ma_20'] = pred2
f = open(data_pat + '/linear_regress_22/ma_20/fac.pkl', 'wb')  # 记得修改
pickle.dump(fac, f, -1)
f.close()
...
fac['ma_60'] = pred3
f = open(data_pat + '/linear_regress_22/ma_60/fac.pkl', 'wb')  # 记得修改
pickle.dump(fac, f, -1)
f.close()
...
fac['ma_120'] = pred4
f = open(data_pat + '/linear_regress_22/ma_120/fac.pkl', 'wb')  # 记得修改
pickle.dump(fac, f, -1)
f.close()
...
fac['ma_240'] = pred5
f = open(data_pat + '/linear_regress_22/ma_240/fac.pkl', 'wb')  # 记得修改
pickle.dump(fac, f, -1)
f.close()
