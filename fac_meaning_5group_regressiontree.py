...
def pool_tree_pred(ro_wind, pre_wind):
    ...
    for i in np.arange((ro_wind + pre_wind), len(trade_days), 1):
        ...
        tr = DecisionTreeRegressor(max_depth=depth_m).fit(sub_data.iloc[:, 0:-1], sub_data.iloc[:, -1])
        coef_param[trade_days[i]] = pd.Series(tr.feature_importances_, index=sub_data.iloc[:, 0:-1].columns)  # 保留参数
        print("correct rate: ", tr.score(sub_data.iloc[:, 0:-1], sub_data.iloc[:, -1]))

        test_data = new_f.loc[pd.to_datetime(trade_days[i]), :]  # 参数隔（pred_window+1）天后才能用
        test_data = test_data.drop(['stock_rela'], axis=1)
        prediction[trade_days[i]] = pd.Series(tr.predict(test_data), index=test_data.index)
        print(trade_days[i])
...
pred_result = {}
coef_result = {}
pred_result['pool_480_' + str(depth_m)], coef_result['pool_480_' + str(depth_m)] = pool_tree_pred(480, 10)

f = open(data_pat + '/regression_tree/fac_' + str(depth_m) + '.pkl', 'wb')  # 记得修改
pickle.dump(pred_result, f, -1)
f.close()
f = open(data_pat + '/regression_tree/coef_' + str(depth_m) + '.pkl', 'wb')  # 记得修改
