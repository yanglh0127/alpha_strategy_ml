...
def pool_lasso_pred(ro_wind, pre_wind, alp):
    prediction = {}
    coef = {}
...
pred_result['pool_480_' + str(lasso_cons)], coef_result['pool_480_' + str(lasso_cons)], feature_result['pool_480_' + str(lasso_cons)] = pool_lasso_pred(480, 10, lasso_cons)

f = open(data_pat + '/lasso/fac_' + str(lasso_cons) + '.pkl', 'wb')  # 记得修改
pickle.dump(pred_result, f, -1)
f.close()
f = open(data_pat + '/lasso/coef_' + str(lasso_cons) + '.pkl', 'wb')  # 记得修改
pickle.dump(coef_result, f, -1)
f.close()
f = open(data_pat + '/lasso/feat_' + str(lasso_cons) + '.pkl', 'wb')  # 记得修改
