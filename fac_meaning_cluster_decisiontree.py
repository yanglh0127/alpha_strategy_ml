...
new_f['next_re_isup'] = (new_f['next_re'].groupby(new_f.index.get_level_values(0)).rank(pct=True) >= 0.5).astype(int)  # 收益率处于最高的前50%标志
...
for i in range(len(trade_days) - 1):
    if i >= win:
        ...
        clf = DecisionTreeClassifier(max_depth=5, min_samples_split=100, min_samples_leaf=50).fit(x.values, y.values)
        print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
...
f = open(data_pat + '/decision_tree/window_' + str(win) + '/fac.pkl', 'wb')  # 记得修改
pickle.dump(fac, f, -1)
f.close()
