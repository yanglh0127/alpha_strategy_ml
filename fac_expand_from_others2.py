...
"""
"""
# 因子聚合方式（五）：同一类别下取sharpe比率最高的前30%进行等权聚合
fac_comb = {}
for type, v in all_fac.items():
    for tag, fac_names in v.items():
        fac_names = [fa for fa in fac_names if fa not in ['factor_20216_vp', 'factor_90007_daily_vp']]  # 这两个因子似乎略有问题
        fac_names = [fa for fa in fac_names if sharpe_rank[type].loc[fa] >= 0.7]  # 选出夏普比率排名前30%的
        print(type, tag, len(fac_names))
        if len(fac_names) > 0:
            temp = {}
            for fac_name in fac_names:
            ...
            comb = pd.concat(temp.values())
            print('mean')
            fac_comb['30%_eq_1_' + tag + '_' + type] = comb.groupby(comb.index).mean()
            fac_comb['30%_eq_1_' + tag + '_' + type].index = pd.to_datetime(fac_comb['30%_eq_1_' + tag + '_' + type].index)

f = open(data_pat + '/fac_expand/30%_eq/fac.pkl', 'wb')
pickle.dump(fac_comb, f, -1)
f.close()
"""
...
for type, v in all_fac.items():
    for tag, fac_names in v.items():
        fac_names = [fa for fa in fac_names if fa not in ['factor_20216_vp', 'factor_90007_daily_vp']]  # 这两个因子似乎略有问题
        fac_names = [fa for fa in fac_names if sharpe_rank[type].loc[fa] >= 0.85]  # 选出夏普比率排名前15%的
        print(type, tag, len(fac_names))
        if len(fac_names) > 0:
            temp = {}
            for fac_name in fac_names:
            ...
            comb = pd.concat(temp.values())
            print('mean')
            fac_comb['15%_eq_1_' + tag + '_' + type] = comb.groupby(comb.index).mean()
            fac_comb['15%_eq_1_' + tag + '_' + type].index = pd.to_datetime(fac_comb['15%_eq_1_' + tag + '_' + type].index)
