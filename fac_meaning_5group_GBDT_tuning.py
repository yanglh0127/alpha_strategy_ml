...
def pool_tree_pred(ro_wind, pre_wind):
    ...
    prediction = {}
    coef_param = {}
    update_time = np.arange((ro_wind + pre_wind), len(trade_days), 20)  # 隔240天更新一下权重
    for i in np.arange((ro_wind + pre_wind), len(trade_days), 1):
        ...
        if i in update_time:
            ...
            for dep in depth_m:
                for fea in feat_m:
                    rf = GradientBoostingRegressor(loss='huber', learning_rate=learn_v,
                                                   n_estimators=tree_num, max_depth=dep, max_features=fea).fit(train_data.iloc[:, 0:-1], train_data.iloc[:, -1])
...
pred_result = {}
coef_result = {}
pred_result['pool_480_' + str(tree_num) + '_' + str(depth_m) + '_' + str(learn_v) + '_' + str(feat_m)], coef_result['pool_480_' + str(tree_num) + '_' + str(depth_m) + '_' + str(learn_v) + '_' + str(feat_m)] = pool_tree_pred(480, 10)

f = open(data_pat + '/gradient_boost_tune/fac_' + str(tree_num) + '_' + str(depth_m) + '_' + str(learn_v) + '_' + str(feat_m) + '.pkl', 'wb')  # 记得修改
pickle.dump(pred_result, f, -1)
f.close()
f = open(data_pat + '/gradient_boost_tune/coef_' + str(tree_num) + '_' + str(depth_m) + '_' + str(learn_v) + '_' + str(feat_m) + '.pkl', 'wb')  # 记得修改
