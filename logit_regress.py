...
# logreg = LogisticRegression(C=0.1, penalty='elasticnet', solver='saga', l1_ratio=0.8)
for i in np.arange((roll_window + pred_window), len(ud_tag) + 1, 1):
    ...
    a.columns = ['ud']
    # new_df = new_f.loc[ud_tag.index[(i - roll_window - pred_window):(i - pred_window)]]
    ...
    print(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
    print("correct rate: ", logreg.score(x.values, y.values))
    pred = logreg.predict_proba(test_df.values)[:, 1]  # 未来n日涨的概率的预测值
...
f = open('E:/FT_Users/LihaiYang/Files/factor_comb_data/ml_comb/logit_regress/240_1.pkl', 'wb')  # 记得修改
